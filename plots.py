...
def true_predicted(y_true, y_pred, agg='mean', quantile=True, ms=None, ax=None):
    ...
    if ms is None:
        if c_quantile is not None:
            cbar = ax.scatter(y_true, y_pred_point, c=c_quantile, cmap='coolwarm', zorder=2)  #(x軸, y軸, c:資料點的顏色, cmap:顏色地圖)
        else:
            cbar = ax.scatter(y_true, y_pred_point, zorder=2)
    else:
    ...
    # 計算 MSE
    mse = metrics.mean_squared_error(y_true, y_pred_point)
    ax.text(limits[0], limits[1], f"$R^2={r2:.2f}$\n$MSE={mse:.4f}$", 
            va='top', ha='left', fontsize=12, bbox=dict(facecolor='white', alpha=0.7))
