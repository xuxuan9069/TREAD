...
class NormalizedScaleEmbedding(nn.Module):
    def __init__(self, input_shape, mlp_dims, downsample=1, eps=1e-8):
        ...
        self.MLP = MLP(input_shape=(865,), dims=mlp_dims) #台灣資料：865、日本資料：2033
    ...
    def forward(self, x):  #(250, 3000, 6)
        ...
        x = torch.cat((x, scale), -1)  #(250, 865) #日本資料torch.Size([700, 2033])
...
class TotalEmbedding(nn.Module):
    ...
    def forward(self, waveforms, inputs, targets): #(64, 25, 3000, 6)
        ...
        targets_mask = torch.unsqueeze(torch.any(torch.not_equal(targets,0),-1),-1).to(self.device)

        tmp = np.concatenate([np.ones((inputs.shape[0],inputs.shape[1])), np.zeros((inputs.shape[0],targets.shape[1]))], axis=1)
        ...
        targets = self.Target_Station_emb(targets)
        targets = targets * targets_mask

        outputs = torch.cat((outputs, targets), 1)
        input_station_mask = torch.logical_or(waveforms_mask, inputs_mask)
        station_mask = torch.cat((input_station_mask, targets_mask),1).to(self.device)
...
class TransformerLayer(nn.Module):
    def __init__(self, total_station, hidden_dropout=0.0,  mlp_dims=(500,500,500), device='cpu', 
        ...
        # self.MultiHeadSelfAttention = nn.MultiheadAttention(500, 10,batch_first=True)
    ...
    def forward(self, x, att_mask, station_mask):
        # att_mask = att_mask.unsqueeze(2).repeat(10,1,275)
        # modified_x,_= self.MultiHeadSelfAttention(query=x,key=x,value=x, attn_mask=att_mask)
...
class ConvolutionModule(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        x = x.transpose(1, 2)
        x = self.sequential(x)
        x = torch.add(input, x.transpose(1, 2))
        return x
...
class ConformerLayer(nn.Module):
    def __init__(self, total_station, hidden_dropout=0.0,  mlp_dims=(500,500,500), device='cpu', 
        ...
        self.hidden_dropout = hidden_dropout
        self.PointwiseFeedForward1= PointwiseFeedForward(emb_dim=mlp_dims[-1], device=device, **ffn_params)
        self.MultiHeadSelfAttention = MultiHeadSelfAttention(total_station=total_station, mlp_dims=mlp_dims, device=device, **mad_params)
        ...
        self.PointwiseFeedForward2= PointwiseFeedForward(emb_dim=mlp_dims[-1], device=device, **ffn_params)
        self.LayerNorm1 = LayerNorm(input_shape=mlp_dims, **norm_params)

    def forward(self, x, att_mask, station_mask): 
        
        x = x + self.PointwiseFeedForward1(x,station_mask)/2
        
        modified_x = self.MultiHeadSelfAttention(x, att_mask, station_mask)
        
        modified_x_input = self.ConvModule(modified_x[:,:25,:] + x[:,:25,:])
        modified_x = torch.cat([modified_x_input,modified_x[:,25:,:]],dim=1)
        if self.hidden_dropout > 0:
            modified_x = nn.Dropout(self.hidden_dropout)(modified_x)
        
        modified_x = modified_x + self.PointwiseFeedForward2(x,station_mask)/2
        
        if self.hidden_dropout > 0:
            modified_x = nn.Dropout(self.hidden_dropout)(modified_x)
            
        x = self.LayerNorm1(modified_x)
        return x
...
class Conformer(nn.Module):
    def __init__(self, device, total_station=45, layers=6, hidden_dropout=0.0,  mlp_dims=(500,500,500),
        ...
        tmp = ConformerLayer(total_station=total_station, hidden_dropout=hidden_dropout, device=device,
                                mlp_dims=mlp_dims, mad_params=mad_params, ffn_params=ffn_params, norm_params=norm_params)
        self.layers = nn.ModuleList([copy.deepcopy(tmp) for i in range(layers)])
...
class DistanceNetwork_v1(nn.Module): #v1
    def __init__(self, n_pga_targets, input_dim=500, hidden_dim=128):
        super(DistanceNetwork_v1, self).__init__()
        self.n_pga_targets = n_pga_targets
        self.fc = torch.nn.Sequential(
            torch.nn.Linear(input_dim * 2, hidden_dim),
            torch.nn.Linear(hidden_dim, 1),  # 預測單一距離值
            torch.nn.ReLU()
        )
        ...
        for layer in self.fc:
            if isinstance(layer, nn.Linear):
                ...
                nn.init.zeros_(layer.bias)  # 偏置初始化為 0
        
    def forward(self, inputs):
        """
        inputs: (batch_size, num_stations + num_targets, 500)
        輸入包含測站與目標的特徵，模型自動計算兩者的距離。
        """
        batch_size, total_points, _ = inputs.shape

        # 分離測站與目標
        stations = inputs[:, :total_points-self.n_pga_targets, :]  # (batch_size, num_stations, 500)
        targets = inputs[:, -self.n_pga_targets:, :]   # (batch_size, num_targets, 500)

        # 使用 broadcasting 生成 (station, target) 的所有組合
        stations_expanded = stations.unsqueeze(2)  # (batch_size, num_stations, 1, 500)
        targets_expanded = targets.unsqueeze(1)    # (batch_size, 1, num_targets, 500)

        # 使用 broadcasting 產生 (batch_size, num_stations, num_targets, 500)
        concat_features = torch.cat(
            [stations_expanded.expand(-1, -1, targets.size(1), -1), 
             targets_expanded.expand(-1, stations.size(1), -1, -1)],
            dim=-1
        )

        # reshape 為 (batch_size * num_stations * num_targets, 1000)
        concat_features = concat_features.view(-1, concat_features.size(-1))

        # 經過 fully connected network 預測距離
        predicted_distances = self.fc(concat_features)  # (batch_size * num_stations * num_targets, 1)

        # reshape 為 (batch_size, num_stations * num_targets)
        predicted_distances = predicted_distances.view(
            stations.size(0), -1
        )

        return predicted_distances
    
class DistanceNetwork_v2(nn.Module): #v2
    def __init__(self, n_pga_targets, input_dim=500, hidden_dim=128):
        super(DistanceNetwork_v2, self).__init__()
        self.n_pga_targets = n_pga_targets
        
        # 訓練中的轉換矩陣 (500 x 500)
        self.transformation_matrix = nn.Parameter(torch.randn(input_dim, input_dim) * 0.01)
    ...
    def forward(self, inputs):
        ...
        # 先對測站 embedding 進行線性變換
        stations_transformed = torch.matmul(stations, self.transformation_matrix)  # (batch_size, num_stations, 500)
        
        # 使用 broadcasting 生成 (station, target) 的所有組合
        stations_expanded = stations_transformed.unsqueeze(2)  # (batch_size, num_stations, 1, 500)
        targets_expanded = targets.unsqueeze(1)  # (batch_size, 1, num_targets, 500)
        
        # 進行 element-wise 乘法，使兩者交互作用
        interaction_features = stations_expanded * targets_expanded  # (batch_size, num_stations, num_targets, 500)
        
        # reshape 為 (batch_size * num_stations * num_targets, 500)
        interaction_features = interaction_features.view(-1, interaction_features.size(-1))
        
        # 經過 fully connected network 預測距離
        predicted_distances = self.fc(interaction_features)  # (batch_size * num_stations * num_targets, 1)
        
        # reshape 為 (batch_size, num_stations * num_targets)
        predicted_distances = predicted_distances.view(batch_size, -1)
...
class FullModel(nn.Module):
    def __init__(self, device, input_shape, downsample, mlp_dims, wavelength, total_station,
                 ...
                 mad_params={}, ffn_params={},model_type="transformer",distance_type="v1"):
        ...
        if(model_type=="transformer"):
            self.Transformer = Transformer(total_station=total_station, device=device, layers=layers, hidden_dropout=hidden_dropout, mlp_dims=mlp_dims,
                                        mad_params=mad_params, ffn_params=ffn_params)
        else:
            self.Transformer = Conformer(total_station=total_station, device=device, layers=layers, hidden_dropout=hidden_dropout, mlp_dims=mlp_dims,
                                        mad_params=mad_params, ffn_params=ffn_params)

        print(f"distance_type:{distance_type}")
        if distance_type=="v1":
            self.DistanceNetwork = DistanceNetwork_v1(n_pga_targets=n_pga_targets, input_dim=500, hidden_dim=128)
        elif distance_type=="v2":  
            self.DistanceNetwork = DistanceNetwork_v2(n_pga_targets=n_pga_targets, input_dim=500, hidden_dim=128)
    ...
    def forward(self, waveforms, inputs, targets):
        x, att_mask, station_mask = self.TotalEmbedding(waveforms, inputs, targets)
        x = self.Transformer(x, att_mask, station_mask)
        distance = self.DistanceNetwork(x)
...
def build_transformer_model(max_stations,
    ...
    parameters = {'input_shape':input_shape, 'downsample':downsample, 'mlp_dims':waveform_model_dims,
                  ...
                  'mad_params':mad_params, 'ffn_params':ffn_params, 'device':device,'model_type':model,'distance_type':distance_type,
                 }
...
def distance_label(station_coords, target_coords):
    R = 6371  # 地球半徑，單位：公里
...
class EnsembleEvaluateModel:
    def __init__(self, config, experiment_path, max_ensemble_size=None, loss_limit=None, batch_size=64, device='cpu'):
        ...
        # for ens_id in [0,1,2,3,8]:
            ...
            # pytorch_total_params = sum(p.numel() for p in tmp_model.parameters())
            # print(f"total_params:{pytorch_total_params}")
            
            tmp_model.load_state_dict(torch.load(weight_file, map_location=self.device)['model_weights'])
            
            self.models = self.models + [tmp_model]
        self.loss_limit = loss_limit

    def predict_generator(self, generator, **kwargs):
        event_id = kwargs.get('event_id', None)  # 如果沒有這個 key，預設回傳 None
        ...
        pga_labels = torch.tensor([], device=self.device) 
        distance_labels = torch.tensor([], device=self.device)
        ...
        for i, model in enumerate(self.models):
            ...
            tmp_distances = torch.tensor([], device=self.device)
            tmp_pga_labels = torch.tensor([], device=self.device)
            tmp_distance_labels = torch.tensor([], device=self.device)
            
            model.eval()
            with torch.no_grad():
                for data_idx, data in tqdm(enumerate(dataloader), total=len(dataloader), desc='Model Pred...'):
                    waveforms = data[0][0].to(self.device)
                    inputs = data[0][1].to(self.device)
                    targets = data[0][2].to(self.device)
                    labels = data[1][0].to(self.device)
                    
                    outputs, pred_distance = model(waveforms, inputs, targets)   #(batch, 20, 5, 3)

                    tmp_preds = torch.cat((tmp_preds, outputs), 0)
                    tmp_distances = torch.cat((tmp_distances, pred_distance), 0) #(batch_size, 750=50*15)
                    tmp_pga_labels = torch.cat((tmp_pga_labels, labels), 0)
                    tmp_distance_labels = torch.cat((tmp_distance_labels, distance_label(inputs, targets)), 0)
                
            tmp_preds = torch.unsqueeze(tmp_preds, 0) #(10197, 15, 5, 3)->(1, 10197, 15, 5, 3)
            preds = torch.cat((preds, tmp_preds), 0)  # torch.Size([10 ensemble次數, 2903, 20, 5, 3])=(1, 10197, 15, 5, 3)
            tmp_distances = torch.unsqueeze(tmp_distances, 0) #(10197, 750=50*15)->(1, 10197, 750)
            distances = torch.cat((distances, tmp_distances), 0) #(1, 10197, 750)
            tmp_pga_labels = torch.unsqueeze(tmp_pga_labels, 0) 
            pga_labels = torch.cat((pga_labels, tmp_pga_labels), 0)
            tmp_distance_labels = torch.unsqueeze(tmp_distance_labels, 0) 
            distance_labels = torch.cat((distance_labels, tmp_distance_labels), 0)
            
        preds = preds.cpu().numpy()
        distances = distances.cpu().numpy()
        pga_labels = pga_labels.cpu().numpy()
        distance_labels = distance_labels.cpu().numpy()
        
        return self.merge_preds(preds, distances, pga_labels, distance_labels)

    @staticmethod
    def merge_preds(preds, distances, pga_labels, distance_labels):
        ...
        distance_item = np.concatenate(distances, axis=0)  # (10, 2903, 750) --> (29030, 750)
        # distance_item /= np.sum(distance_item, axis=-1, keepdims=True)  # (29030, 750) 歸一化

        pga_labels_item = np.concatenate(pga_labels, axis=0)
        distance_labels_item = np.concatenate(distance_labels, axis=0)
        
        return pred_item, distance_item, pga_labels_item, distance_labels_item
